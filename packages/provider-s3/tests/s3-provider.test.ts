...
import { createS3Provider } from "../src/s3-provider.js";
...
describe("createS3Provider", () => {
  it("create() calls a presigned URL for the operation and parses the UploadId", async () => {
    ...
    const getPresignedUrl = vi.fn(async () => ({
      url: "https://s3.example/bucket/key?uploads",
    }));
    ...
    expect(request.url).toBe("https://s3.example/bucket/key?uploads");
  });
  ...
  it("uploadPart() requests a part-specific presigned URL and reads the ETag response header", async () => {
    ...
    const getPresignedUrl = vi.fn(async () => ({
      url: "https://s3.example/bucket/key?partNumber=3&uploadId=abc",
    }));
    const provider = createS3Provider({
      getPresignedUrl,
    });
  ...
  it("complete() sends a CompleteMultipartUpload XML body listing every part", async () => {
    const send = vi.fn(
      async (_request: TransportRequest): Promise<TransportResponse> =>
        xmlResponse(
          200,
          '<CompleteMultipartUploadResult><Location>https://s3.example/bucket/key</Location><ETag>"final-etag"</ETag></CompleteMultipartUploadResult>',
        ),
    );
    const transport: UploadTransport = {
      send,
