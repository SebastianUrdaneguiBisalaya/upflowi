...
(() => {
  ...
  let annotLastPinClick = {
    idx: -1,
    time: 0,
  }; // for click-click-to-delete
  ...
  function onAnnotMove(e) {
    ...
    annotPointer.strokePoints.push([
      p.x,
      p.y,
    ]);
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      classes: el.classList
        ? [
            ...el.classList,
          ].filter((cls) => cls.indexOf("impeccable-") !== 0)
        : [],
      id: el.id || null,
      newText,
      originalText,
  ...
  function componentModuleCandidates(manifest, modulePath, absPath) {
    ...
    const candidates = [
      new URL(base + rel, location.origin).href,
    ];
  ...
  async function probePreviewTree(manifest) {
    ...
    try {
      ...
      return {
        ok: false,
        tried: err.impeccableTriedUrls || candidates,
      };
    }
  }
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      svelteComponentSession.mountedInstance = runtime.mount(Component, {
        intro: false,
        props: {
          ...svelteComponentSession.propValues,
        },
    ...
    for (const sourceNode of sourceNodes) {
      ...
      if (tokens.length === 1) {
        ...
        const match = liveText.match(
          expressionTextMatcher(sourceText, [
            token,
          ]),
        );
  ...
  function watchForDiscardedFrameworkWrapperRemoval(sessionId) {
    ...
    timer = setTimeout(resolveStillMounted, 12000);
    discardedFrameworkWrapperWatchers.set(sessionId, {
      observer,
      timer,
    });
  }
...
void main() {
  ...
  function watchForHandledRuntimeWrapper(
    ...
    handledRuntimeWrapperWatchers.set(sessionId, {
      observer,
      timer,
    });
    findHandledWrapper();
  ...
  function focusConfigureInput(reason) {
    steerFocusLog("focusConfigureInput", {
      reason,
    });
    const inputId =
  ...
  function focusSteerChat(reason) {
    steerFocusLog("focusSteerChat called", {
      reason,
    });
  ...
  function syncPageChatFocus(reason) {
    steerFocusLog("syncPageChatFocus", {
      reason,
    });
